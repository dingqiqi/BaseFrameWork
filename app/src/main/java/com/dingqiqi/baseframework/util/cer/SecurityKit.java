...
import com.dingqiqi.baseframework.util.cer.CertInfoModual;
...
final class SecurityKit {
    ...
    public CertInfoModual getCertInfoByKeyStore(String certificatePath, String password) throws Exception {
        ...
        for (int i = 0; i < commonName.length; i++) {
            if (commonName[i].startsWith("CN=")) {
                certInfo.setCommonName(commonName[i].substring(3,
                        commonName[i].length()));
            }
        }
        return certInfo;
     ...
     * @throws Exception
     */
    public PublicKey getPublicKeyByCertificate(String certificatePath)
            throws Exception {
        // 获得证书
     ...
     *
     * @param data     待加密数据
     * @param certPath 服务器公钥文件路径
     * @return byte[] 加密数据
     * @throws Exception
     */
    public byte[] encryptByPublicKey(byte[] data, String certPath)
            throws Exception {
     ...
     * @throws Exception
     */
    public byte[] decryptByPublicKey(byte[] data, String certPath)
            throws Exception {
     ...
     * @throws Exception
     */
    public byte[] sign(byte[] data, CertInfoModual certInfoModual)
            throws Exception {
        // 构建签名，由证书指定签名算法
//        Signature signature = Signature.getInstance("SHA1withRSA");
     ...
     * @throws Exception
     */
    public boolean verify(byte[] data, byte[] signdata, String certPath)
            throws Exception {
        // 获取证书
