...
public abstract class BaseAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public BaseAdapter(Context mContext, List<? extends BaseMode> mList) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType <= HEAD_TAG) {
            ...
            if (view == null) {
                view = getCustomItemView(parent, HEAD_TAG);
            }
            ...
            if (view == null) {
                ...
                mHolder = getCustomItemHolder(view, HEAD_TAG);
            }
            ...
            if (view == null) {
                view = getCustomItemView(parent, FOOT_TAG);
            }
            ...
            if (view == null) {
                ...
                mHolder = getCustomItemHolder(view, FOOT_TAG);
            }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (getItemViewType(position) <= HEAD_TAG) {
            if (holder instanceof HeadHolder) {
                onItemBindHolder(holder, position, HEAD_TAG);
            }
        } else if (getItemViewType(position) == BODY_TAG) {
            if (holder instanceof ItemHolder) {
                onItemBindHolder(holder, position - mHeadViews.size(), BODY_TAG);
            }
        } else if (getItemViewType(position) <= FOOT_TAG) {
            if (holder instanceof FooterHolder) {
                onItemBindHolder(holder, position - mHeadViews.size() - mList.size(), FOOT_TAG);
            }
        } else {
            if (holder instanceof MoreHolder) {
                onItemBindHolder(holder, getItemCount() - 1, MORE_TAG);
            }
        }
    }
        ...
        if (recyclerView.getLayoutManager() instanceof GridLayoutManager) {
            ...
            manager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
                ...
                public int getSpanSize(int position) {
                    return getItemViewType(position) == BODY_TAG ? 1 : manager.getSpanCount();
                }
            });
        }
    }
    ...
    protected void handleLayoutIfStaggeredGridLayout(RecyclerView.ViewHolder holder, int position) {
        if (getItemViewType(position) != BODY_TAG) {
